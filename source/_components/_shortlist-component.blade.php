	...
	<div class="flex-1 bg-gray-200 m-4 p-6 rounded-md">
		...
		<ul class="mt-3 ml-4 sm:ml-6 md:ml-6 list-disc">
			@foreach($shortlist->nominees as $nominee)
				<li class="inline-flex items-center w-full text-md">
		...
		@if($shortlist->sponsor)
			...
			<div class="mt-4 flex justify-center">
				<img class="h-20 w-auto max-w-sm" src="{{ $page->baseUrl }}/assets/images/sponsors/awards/{{ $shortlist->sponsor }}.svg" alt="{{ $shortlist->sponsor }}">
			</div>
		@endif
	</div>
</div>
